        ...
        with torch.backends.cuda.sdp_kernel(enable_flash=True, enable_math=True, enable_mem_efficient=True):  # enable flash attention for faster inference
            with torch.no_grad():
                if tempimage:
                    ...
                    multimodal_model = await asyncio.to_thread(LlavaForConditionalGeneration.from_pretrained, multimodal_name, torch_dtype=torch.float16, low_cpu_mem_usage=True, load_in_4bit=True, bnb_4bit_compute_dtype=torch.float16)
