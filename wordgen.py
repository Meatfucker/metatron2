...
async def llm_generate(message, prompt, model, tokenizer, system_prompt, negative_prompt):
    ...
    negative_input_ids = tokenizer.encode(negative_prompt, return_tensors="pt") #turn negative prompt into tokens
    ...
    await save_history(generated_text, message, system_prompt) #save the response to the users history
    return(llm_response)
    
async def save_history(generated_text, message, system_prompt):
...
async def insert_history(userid, prompt, llm_prompt, system_prompt):
    ...
    if userid not in wordgen_user_history: #if they have no history, include the system prompt
        ...
        injectedhistory = f'{system_prompt}\n\nUSER:{prompt}\nASSISTANT:{llm_prompt}</s>\n'
    else:
...
class Wordgenbuttons(discord.ui.View):
    ...
    def __init__(self, generation_queue, userid, message, stripped_message):
        super().__init__()
    ...
    async def delete_history(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Deletes history"""
        if self.userid == interaction.user.id:
            await self.generation_queue.put(('wordgenforget', self.message, self.stripped_message))
